...
public class Ex05StreamMapping {
	...
	public static void main(String[] args) {
		...
		generate(
			"1. Lấy độ dài của từng từ", 
			elements.stream()
					.map(String::length)
					.toList());
		
		generate(
			"2. Lấy độ dài của từng từ kèm giá trị", 
			elements.stream()
		...
		generate(
			"3. Lấy số thứ tự của cùng của tên đĩa thức ăn", 
			menu.stream()
					.map(d -> {
		...
		// source --> Stream<Collection<T>> --> flatMap(Collection::stream) --> Stream<T>

...
public class SusoendResumeDeadlock {
	...
	public static void main(String[] args) throws InterruptedException {
		final Thread thread1 = new Thread("Thread-1") {
			public void run() {
				...
				synchronized (data) {
					 ...
					 * thread, thread will on in waiting state.
					 */
					...
					System.out.println(Thread.currentThread().getId() + " has released lock on String.class");
				}
		...
		Thread thread2 = new Thread("Thread-2") {
			public void run() {
				...
				/*
				 * for acquiring lock on String.class thread-1 must have release lock on
				...
				// check data í lock befor synchronized
				thread1.resume();

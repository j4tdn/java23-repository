...
public class WhAllocationDemo {
	...
	public static void main(String[] args) {
		...
		// Step 7: Apply minimum allocation
		Map<Integer, Integer> minimumAllocation = applyMinimumAllocation(storeMap);
		
		// Step 10-11: Rounding and fix deviation
		BigDecimal planningAmountCountry = BigDecimal.valueOf(DataModel.planningAmount);
	    Map<Integer, Integer> finalAllocation = applyRoundingAndFixDeviation(whAllocationMap, minimumAllocation, storeMap, planningAmountCountry);
		
		// Print results
	    System.out.println("Final Allocation to Warehouses");
	    finalAllocation.forEach((whId, allocation) -> System.out.println("WH " + whId + ": " + allocation + " items"));
	}
	...
	// Step 7 method:
	private static Map<Integer, Integer> applyMinimumAllocation(Map<Integer, Store> storeMap) {
	    Map<Integer, Integer> minAllocation = new HashMap<>();
	    int minPerStore = DataModel.minPerStore;
	    
	    // Apply minimum allocation
	    storeMap.values().forEach(store -> {
	        int whId = store.getWhId();
	        minAllocation.putIfAbsent(whId, 0);
	        minAllocation.put(whId, minAllocation.get(whId) + minPerStore);
	    });
	...
	// Step 11 method:
	private static Map<Integer, Integer> applyRoundingAndFixDeviation(Map<Integer, BigDecimal> whAllocation,
	        Map<Integer, Integer> finalAllocation, Map<Integer, Store> storeMap, BigDecimal planningAmountCountry) {
	    BigDecimal totalPlanned = whAllocation.values().stream().reduce(BigDecimal.ZERO, BigDecimal::add);

	    // If Planning Amount Country < SUM(Allocation Amount Rounded WH of all WH of the country)
	    while (totalPlanned.compareTo(planningAmountCountry) != 0) {
	        ...
	        if (maxDeviationWH.isPresent()) {
	            ...
	            if (deviation.compareTo(BigDecimal.valueOf(DataModel.minPerStore * storePotential.intValue())) == 0) {
	                continue;
	            }

	            finalAllocation.put(whId, finalAllocation.get(whId) - 1); // Subtract one allocation unit
	        }

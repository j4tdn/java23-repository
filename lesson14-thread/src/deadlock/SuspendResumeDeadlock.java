		...
		public static void main(String[] args) throws InterruptedException {
			...
			final Thread thread1 = new Thread("Thread-1") {
				public void run() {
					...
					//synchronized data
					synchronized (data) {
						System.out.println(Thread.currentThread().getName() + "has obtanined lock on data " + " & suspended...");
						
						Thread.currentThread().suspend(); //pause
						System.out.println(Thread.currentThread().getName() + "has started lock on data");
					}
			...
			final Thread thread2 = new Thread("Thread-2") {
				public void run() {
					...
					thread1.resume();
					synchronized (data) {
						System.out.println(Thread.currentThread().getName() + "has obtanined lock on data ");

...
public class Ex03BubbleSortItem {
	...
	public static void main(String[] args) {
		...
		// Override sẵn hàm compareTo() --> int
		//                  so sánh this và o
		
		// 1. Sắp xếp tăng dần theo id
		// Nếu id của pt trước > id pt sau --> Hoán vị
		...
		printf("4. Sorting by salesPrice(desc)", items);
		
		// 5. Sắp xếp giảm dần theo storeId, tăng dần theo expriredDate
		bubbleSort(items, (i1, i2) -> {
		...
		printf("5. Sorting by storeId(desc), expriredDate(asc)", items);
		
		// 6. Sắp xếp tăng dần theo storeId, giảm dần theo salesPrice, giảm dần expriedDate
		bubbleSort(items, (i1, i2) -> {
			Integer storeId1 = i1.getStoreId();
			Integer storeId2 = i2.getStoreId();
			if (storeId1 != storeId2) {
		...
		printf("6. Sorting by storeId(asc), salesPrice(desc), expriredDate(desc)", items);
	}

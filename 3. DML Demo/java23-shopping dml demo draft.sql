...
(1, 'S', 1, 'Size S cho Nam - Từ 10-19 kg'),
...
INSERT INTO T03_ITEM_DETAIL(C03_ITEM_ID, C03_SIZE_ID, C03_COLOR, C03_SALES_PRICE, C03_AMOUNT) VALUES
(1, 1, 'RED', 88, 200),
(1, 2, 'BLUE', 120, 300),
(1, 3, 'GREEN', 127, 100),
(4, 2, 'WHITE', 420, 800),
...
-- rowcount: số dòng cần lấy từ offset
...
SELECT *,
		CAST(C03_SALES_PRICE * 0.77 AS DECIMAL(10,2)) PROMOTION_FEE
...
SELECT C02_SIZE_ID,
       ...
       CASE C02_GENDER
			WHEN 1 THEN 'NAM'
            ELSE 'NỮ'
	   END C02_GENDER, -- if else nhiều trường hợp (hỗ trợ bất kì điều kiện nào),
...
-- IN: trường hợp tập hợp bên trong mệnh đề IN số lượng nhiều  -> gây ra các vấn đề perf or ko hỗ trợ
...
-- Khi dùng GROUP BY, chỉ đc SELECT các columns 
SELECT * FROM T01_ITEM;
...
-- disable/remove only_full_group_by mode --> unexpected
SET SESSION sql_mode=(SELECT REPLACE(@@sql_mode, 'ONLY_FULL_GROUP_BY', ''));

-- enable only_full_group_by mode(chỉ cho fep select columns có xuất hiện trong group by) --> expected
...
select C01_ITEM_ID, C01_ITEM_NAME, C01_ITEM_GROUP_ID from t01_item
group by C01_ITEM_GROUP_ID;
...
-- Mệnh đề UNION
-- Yêu cầu: Liệt kê các mặt hàng(MaMH) thuộc loại hàng là áo và các mặt hàng có giá lớn hơn 100 và bé hơn 300
...
-- Liệt kê các mặt hàng kèm thông tin loại hàng
SELECT t1.*,
	   C04_ITEM_GROUP_NAME
FROM t01_item t1
...
-- INSERT INTO 'java23_shopping'.'t01

-- Liệt kê các mặt hàng có mặt hàng tham chiếu
SELECT *
FROM t01_item
...
-- Liệt kê các mặt hàng có mặt hàng tham chiếu khác mã loại hàng
SELECT item.*,
	   ref.C01_ITEM_NAME REF_ITEM_NAME,
       ref.C01_ITEM_GROUP_ID REF_ITEM_GROUP_ID
FROM t01_item item
...
-- 1. Design Database
-- 2. Create DB patches
-- 3. Create/Refresh Test Data
--    + DELETE/TRUNCATE TABLE DATA

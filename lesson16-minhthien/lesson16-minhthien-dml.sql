...
SELECT *
  FROM T06_ORDER
 WHERE cast(C06_ORDER_TIME AS DATE) = str_to_date('28/11/2019', '%d/%m/%Y');

-- 6. Liệt kê các đơn hàng được bán từ ngày 28/11/2019 đến ngày 02/12/2019
SELECT *
  FROM T06_ORDER
 WHERE cast(C06_ORDER_TIME AS DATE) BETWEEN str_to_date('28/11/2019', '%d/%m/%Y') 
                                        AND str_to_date('02/12/2019', '%d/%m/%Y');

-- 7. Liệt kê các đơn hàng được bán trong tháng 11/2019
SELECT *
  FROM T06_ORDER
 WHERE MONTH(C06_ORDER_TIME) = 11
...
SELECT *,
	   ROUND(C03_SALES_PRICE * 0.8, 2) AS DISCOUNT_PRICE
  FROM T03_ITEM_DETAIL;

-- 10. Giảm giá 20% tất cả các mặt hàng trong ngày 25/11/2019
SELECT t03.*,
	   t06.C06_ORDER_TIME AS ORDER_TIME,
 ...
 WHERE cast(t06.C06_ORDER_TIME AS DATE) = str_to_date('25/11/2019', '%d/%m/%Y');
...
SELECT t01.C01_ITEM_ID AS MaMH,
	   t01.C01_ITEM_NAME AS TenMH,
	   cast(t06.C06_ORDER_TIME AS TIME) AS ThoiGianDatHang
  FROM T06_ORDER t06
  JOIN T16_ORDER_DETAIL t16 ON t16.C16_ORDER_ID = t06.C06_ORDER_ID
  JOIN T03_ITEM_DETAIL t03 ON t03.C03_ITEM_DETAIL_ID = t16.C16_ITEM_DETAIL_ID
  JOIN T01_ITEM t01 ON t01.C01_ITEM_ID = t03.C03_ITEM_ID
 WHERE cast(t06.C06_ORDER_TIME AS DATE) = str_to_date('23/11/2019', '%d/%m/%Y');
